...
def create_NFT_no_contract(algod_client, accts, metadata):
    nft_id = assets.create_NFT(algod_client, accts[0], metadata)
    return nft_id
...
while action == 1:
    ...
    if action == 1:
        ...
        nft_id = create_NFT_no_contract(algod_client, accts, metadata)
        action = 1;
    elif action == 3:
        NFT_transfer(accts, nft_id)
        action = 1;
    elif action == 4:
        account_balance(accts)
        action = 1;
    elif action == 5:
        NFT_delete(accts, nft_if)
        action = 1;
    elif action == 6:
        sys.exit()
        action = 1;
